...
function runClaudeCode(options) {
	...
	if (!result.stdout) {
		log('error', 'ClaudeCode Failed:', result.stderr || "ClaudeCode 运行时出错");
		throw new Error(result.stderr || "ClaudeCode 运行时出错");
	}

	const json = JSON.parse(result.stdout);
	if (json.is_error) {
		log('error', 'ClaudeCode Failed:', result.stderr || "ClaudeCode 返回出错");
...
function runCodex(options) {
	...
	if (result.stderr) result.stderr = result.stderr.toString();
	if (!result.stdout) {
		log('error', 'Codex Failed:', result.stderr || "Codex 运行时出错");
...
function runGemini(options) {
	...
	if (result.stderr) result.stderr = result.stderr.toString();
	if (!result.stdout) {
		log('error', 'Gemini Failed:', result.stderr || "Codex 运行时出错");
		throw new Error(result.stderr || "Codex 运行时出错");
	}
...
try {
	const reply = runGemini({
		systemPrompt: "你是我的情人徐泓～～",
		prompt: "亲爱的，我想你啦！",
	});
	console.log(reply);
}
