...
const parseToolUse = (data) => {
	...
	if (toolName === 'Task') {
		...
		toolInfo = ['Todos:'];
		data.tool_input.todos.forEach(item => {
			if (item.status === 'completed') toolInfo.push('　 ✔︎ ' + item.content);
			else if (item.status === 'pending') toolInfo.push('　 ・ ' + item.content);
			else if (item.status === 'in_progress') toolInfo.push('　 ☛ ' + item.content);
			else toolInfo.push('　 ✗ ' + item.content);
		});
		toolInfo = toolInfo.join('\n');
	}
	else if (toolName === 'Skill') {
		toolInfo = `Skill (${data.tool_input.skill})`;
	}
...
module.exports = {
	...
	prepareEnvironment, cccoreSocket,
	readStdin,
};
